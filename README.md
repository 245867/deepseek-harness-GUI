...
[![License](https://img.shields.io/badge/License-MIT-2E7D32?style=for-the-badge&logo=opensourceinitiative&logoColor=white)](LICENSE)
...
<p align="center">
  <img src="docs/images/主界面.png" alt="DSH-GUI 主界面" width="820">
</p>

上图是启动后的主界面。左侧一列是会话列表与工作台入口（完整工作台、插件、设置与模型、关于 DSH-GUI），右侧是对话区与输入框；底部状态栏实时显示网关地址与连接状态，以及模型和权限的加载情况。

刚打开时是欢迎页，页面上给出「分析项目结构」「设计实现方案」「解释代码」三个快捷入口，点一下即可发起一次会话；左下角的模型入口用来选择 `models.yaml` 里配置好的模型分组。
